...
(`util/topgen.py`). Please do not revise `rtl/top_tetley.sv`, the crossbar
...
Top module `rtl/top_tetley.sv` is created by `topgen.py`. Current top module
is created with below command.

```console
../../util/topgen.py -t data/top_tetley.hjson -c data -o . -v
```

It generates files below:

- `rtl/top_tetley.sv`: Top module generated from the template
    `data/top_tetley.sv.tpl` with the configuration file
    `data/top_tetley.hjson`
- `rtl/xbar_main.sv` and `rtl/tl_main_pkg.sv`: Crossbar module. As of now,
    tetley has only one main crossbar. tlgen library is used to generate
    these files.
...
Modify `data/top_tetley.hjson` to include new module
...
Main configuration for Top Earlgrey is in `data/top_tetley.hjson`. The users
...
Main top template file is `data/top_tetley.sv.tpl`. In most cases, it isn't
