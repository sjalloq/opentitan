...
module rv_plic_reg_top (
  ...
  always_comb begin
    ...
    addr_hit[64] = (reg_addr == RV_PLIC_IE00_OFFSET);
    addr_hit[65] = (reg_addr == RV_PLIC_IE01_OFFSET);
    addr_hit[66] = (reg_addr == RV_PLIC_THRESHOLD0_OFFSET);
    addr_hit[67] = (reg_addr == RV_PLIC_CC0_OFFSET);
  ...
  assign ie00_e0_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e0_wd = reg_wdata[0];

  assign ie00_e1_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e1_wd = reg_wdata[1];

  assign ie00_e2_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e2_wd = reg_wdata[2];

  assign ie00_e3_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e3_wd = reg_wdata[3];

  assign ie00_e4_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e4_wd = reg_wdata[4];

  assign ie00_e5_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e5_wd = reg_wdata[5];

  assign ie00_e6_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e6_wd = reg_wdata[6];

  assign ie00_e7_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e7_wd = reg_wdata[7];

  assign ie00_e8_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e8_wd = reg_wdata[8];

  assign ie00_e9_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e9_wd = reg_wdata[9];

  assign ie00_e10_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e10_wd = reg_wdata[10];

  assign ie00_e11_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e11_wd = reg_wdata[11];

  assign ie00_e12_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e12_wd = reg_wdata[12];

  assign ie00_e13_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e13_wd = reg_wdata[13];

  assign ie00_e14_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e14_wd = reg_wdata[14];

  assign ie00_e15_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e15_wd = reg_wdata[15];

  assign ie00_e16_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e16_wd = reg_wdata[16];

  assign ie00_e17_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e17_wd = reg_wdata[17];

  assign ie00_e18_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e18_wd = reg_wdata[18];

  assign ie00_e19_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e19_wd = reg_wdata[19];

  assign ie00_e20_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e20_wd = reg_wdata[20];

  assign ie00_e21_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e21_wd = reg_wdata[21];

  assign ie00_e22_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e22_wd = reg_wdata[22];

  assign ie00_e23_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e23_wd = reg_wdata[23];

  assign ie00_e24_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e24_wd = reg_wdata[24];

  assign ie00_e25_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e25_wd = reg_wdata[25];

  assign ie00_e26_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e26_wd = reg_wdata[26];

  assign ie00_e27_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e27_wd = reg_wdata[27];

  assign ie00_e28_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e28_wd = reg_wdata[28];

  assign ie00_e29_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e29_wd = reg_wdata[29];

  assign ie00_e30_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e30_wd = reg_wdata[30];

  assign ie00_e31_we = addr_hit[64] & reg_we & ~wr_err;
  assign ie00_e31_wd = reg_wdata[31];

  assign ie01_e32_we = addr_hit[65] & reg_we & ~wr_err;
  assign ie01_e32_wd = reg_wdata[0];

  assign ie01_e33_we = addr_hit[65] & reg_we & ~wr_err;
  assign ie01_e33_wd = reg_wdata[1];

  assign ie01_e34_we = addr_hit[65] & reg_we & ~wr_err;
  assign ie01_e34_wd = reg_wdata[2];

  assign ie01_e35_we = addr_hit[65] & reg_we & ~wr_err;
  assign ie01_e35_wd = reg_wdata[3];

  assign ie01_e36_we = addr_hit[65] & reg_we & ~wr_err;
  assign ie01_e36_wd = reg_wdata[4];

  assign ie01_e37_we = addr_hit[65] & reg_we & ~wr_err;
  assign ie01_e37_wd = reg_wdata[5];

  assign ie01_e38_we = addr_hit[65] & reg_we & ~wr_err;
  assign ie01_e38_wd = reg_wdata[6];

  assign ie01_e39_we = addr_hit[65] & reg_we & ~wr_err;
  assign ie01_e39_wd = reg_wdata[7];

  assign ie01_e40_we = addr_hit[65] & reg_we & ~wr_err;
  assign ie01_e40_wd = reg_wdata[8];

  assign ie01_e41_we = addr_hit[65] & reg_we & ~wr_err;
  assign ie01_e41_wd = reg_wdata[9];

  assign ie01_e42_we = addr_hit[65] & reg_we & ~wr_err;
  assign ie01_e42_wd = reg_wdata[10];

  assign ie01_e43_we = addr_hit[65] & reg_we & ~wr_err;
  assign ie01_e43_wd = reg_wdata[11];

  assign ie01_e44_we = addr_hit[65] & reg_we & ~wr_err;
  assign ie01_e44_wd = reg_wdata[12];

  assign ie01_e45_we = addr_hit[65] & reg_we & ~wr_err;
  assign ie01_e45_wd = reg_wdata[13];

  assign ie01_e46_we = addr_hit[65] & reg_we & ~wr_err;
  assign ie01_e46_wd = reg_wdata[14];

  assign ie01_e47_we = addr_hit[65] & reg_we & ~wr_err;
  assign ie01_e47_wd = reg_wdata[15];

  assign ie01_e48_we = addr_hit[65] & reg_we & ~wr_err;
  assign ie01_e48_wd = reg_wdata[16];

  assign ie01_e49_we = addr_hit[65] & reg_we & ~wr_err;
  assign ie01_e49_wd = reg_wdata[17];

  assign ie01_e50_we = addr_hit[65] & reg_we & ~wr_err;
  assign ie01_e50_wd = reg_wdata[18];

  assign ie01_e51_we = addr_hit[65] & reg_we & ~wr_err;
  assign ie01_e51_wd = reg_wdata[19];

  assign ie01_e52_we = addr_hit[65] & reg_we & ~wr_err;
  assign ie01_e52_wd = reg_wdata[20];

  assign ie01_e53_we = addr_hit[65] & reg_we & ~wr_err;
  assign ie01_e53_wd = reg_wdata[21];

  assign ie01_e54_we = addr_hit[65] & reg_we & ~wr_err;
  assign ie01_e54_wd = reg_wdata[22];

  assign ie01_e55_we = addr_hit[65] & reg_we & ~wr_err;
  assign ie01_e55_wd = reg_wdata[23];

  assign ie01_e56_we = addr_hit[65] & reg_we & ~wr_err;
  assign ie01_e56_wd = reg_wdata[24];

  assign ie01_e57_we = addr_hit[65] & reg_we & ~wr_err;
  assign ie01_e57_wd = reg_wdata[25];

  assign ie01_e58_we = addr_hit[65] & reg_we & ~wr_err;
  assign ie01_e58_wd = reg_wdata[26];

  assign ie01_e59_we = addr_hit[65] & reg_we & ~wr_err;
  assign ie01_e59_wd = reg_wdata[27];

  assign threshold0_we = addr_hit[66] & reg_we & ~wr_err;
  assign threshold0_wd = reg_wdata[1:0];

  assign cc0_we = addr_hit[67] & reg_we & ~wr_err;
  assign cc0_wd = reg_wdata[5:0];
  assign cc0_re = addr_hit[67] && reg_re;

  assign msip0_we = addr_hit[68] & reg_we & ~wr_err;
